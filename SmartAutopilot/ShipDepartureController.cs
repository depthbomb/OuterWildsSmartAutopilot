...
internal sealed class ShipDepartureController : IDisposable
{
    ...
    {
        var ready = Ship                                        && cockpit == _cockpit && _landing != null && _landing.IsLanded()
        ...
        var probeConflict = OWInput.UsingGamepad() && Locator.GetProbe() != null && Locator.GetProbe().IsAnchored()
    ...
    public bool TryReadInput(Autopilot pilot, FlightState flight, out Vector3 input)
    {
        ...
        GameFields.LiningUp.Set(pilot, phase is DeparturePhase.Ignition or DeparturePhase.Blocked);
        GameFields.Approaching.Set(pilot, phase is DeparturePhase.LiftOff or DeparturePhase.Climb);
    ...
    private void FindDepartureBody(IReadOnlyList<AstroObject> bodies)
    {
        ...
        var anchor = _surface;
        for (var depth = 0; anchor != null && depth < 8 && _departureBodyIndex < 0; depth++)
        {
    ...
    private bool CheckOverhead(Vector3 position, Vector3 up, float distance)
    {
        ...
        {
            var physicalHull = collider != null    &&
                               collider.enabled    &&
                               !collider.isTrigger &&
        ...
        var overlaps = Physics.OverlapBoxNonAlloc(center, halfExtents, _overlaps, rotation, OWLayerMask.physicalMask, QueryTriggerInteraction.Ignore);
        ...
        var hits = Physics.BoxCastNonAlloc(center, halfExtents, up, _hits, rotation, distance, OWLayerMask.physicalMask, QueryTriggerInteraction.Ignore);
    ...
    private void UpdateIgnition(DeparturePhase phase)
    {
        ...
        if (_ignitionActive && phase != DeparturePhase.Ignition)
        {
            ...
            var complete = phase is DeparturePhase.LiftOff or DeparturePhase.Climb or DeparturePhase.Complete;
            if (complete)
            {
