...
internal sealed class AutomationFault : IDisposable
{
    ...
    public bool Tick(ReferenceFrame target)
    {
        ...
        if (!Result.restored)
        {
            var safe = blocked ? _pilot.IsFlyingToDestination() && _phase == "Hold" : !_pilot.IsFlyingToDestination()               && _ship.GetComponent<ShipThrusterController>().GetTranslationalInput().sqrMagnitude < 0.0001f;
            if (safe)
            {
                if (_safeSince < 0)
                {
            ...
            {
                var needsNotice    = Result.kind != "autopilot-damage";
                var expectedNotice = blocked ? "No clear route" : Result.kind == "fuel-exhaustion" ? "fuel" : "thrust";
