...
namespace SmartAutopilot
{
    internal sealed class ShipArrivalLimits
    {
        ...
        public void Refresh(FlightState state)
        {
            ...
            {
                bool active = volume.Rules != null && volume.Rules.isActiveAndEnabled && volume.Rules.IsVolumeActive();
                if (!active)
                {
                    continue;
                }

                double limit = volume.Rules.GetThrustLimit();
                ...
                foreach (var collider in volume.Colliders)
                {
                    ...
                    double extent = collider is SphereCollider ? Math.Max(bounds.extents.x, Math.Max(bounds.extents.y, bounds.extents.z)) : bounds.extents.magnitude;
