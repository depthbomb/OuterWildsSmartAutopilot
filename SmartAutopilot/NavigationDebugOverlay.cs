...
internal sealed class NavigationDebugOverlay : IDisposable
{
    ...
    public void Draw(FlightState state,
                     FlightComputer computer,
                     FlightCommand command,
                     ShipCollisionGuard collision,
                     ShipDepartureController departure,
                     float refreshRate)
    {
        ...
        var count = computer.GetDebugRoute(state, _route);
        ...
        for (var i = 0; i < state.Obstacles.Count && i < 64; i++)
        {
        ...
        var launching = departure is { Active: true };
        if (launching)
        {
    ...
    private void Sphere(Vector3 center, float radius, Color color)
    {
        ...
        var width = Width(Mathf.Abs(Vector3.Distance(_cameraPosition, center) - radius));
        for (var axis = 0; axis < 3; axis++)
        {
            var line = Line(32, color, true, width, width);
            for (var point = 0; point < 32; point++)
            {
                var x      = _unitRing[point].x * radius;
                var y      = _unitRing[point].y * radius;
