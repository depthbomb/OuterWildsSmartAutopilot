...
namespace SmartAutopilot.Navigation
{
    internal readonly struct CollisionFrame
    {
        ...
        public static CollisionFrame Create(Vector offset, Vector velocity, Vector acceleration, Vector angularVelocity,
            Vector angularAcceleration, double physicalRadius)
        {
            double gap = offset.Length - Math.Max(0, physicalRadius);
            double blend = Math.Max(0, Math.Min(1, (250 - gap) / 200));
            double weight = blend * blend * (3 - 2 * blend);
