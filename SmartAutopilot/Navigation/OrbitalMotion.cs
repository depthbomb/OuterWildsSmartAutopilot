...
namespace SmartAutopilot.Navigation
{
    internal readonly struct OrbitalMotion
    {
        ...
        private OrbitalMotion(Obstacle body, Obstacle primary, int primaryIndex, Vector offset, Vector tangentVelocity)
        {
            ...
            double centripetal = tangentVelocity.LengthSquared / _radius;
        ...
        {
            double angle     = _angularSpeed * time;
        ...
        public bool ExcludesLinearPath(Vector position, Vector velocity, double clearance, double horizon)
        {
            ...
            double closestTime = relativeVelocity.LengthSquared > 1e-9
                ? Math.Max(0, Math.Min(horizon, -Vector.Dot(offset, relativeVelocity) / relativeVelocity.LengthSquared)) : 0;
            // Enclose the whole orbit, with radius uncertainty, instead of projecting a tangent out of it.
            double envelope = _radius * 1.1 + Math.Abs(_radialSpeed) * horizon + clearance;
        ...
        public static bool ExcludesCollision(FlightState state, int index, double clearance, double horizon)
        {
            ...
            bool missingPrimary = index >= 0 && index < state.Obstacles.Count && state.Obstacles[index].PrimaryIndex < 0;
            if (!missingPrimary)
            {
                return false;
            }
            ...
            for (int primary = 0; primary < state.Obstacles.Count; primary++)
            {
        ...
        private bool IsProtected(FlightState state, double clearance, double horizon)
        {
            ...
            double primaryRadius = state.Obstacles[_primaryIndex].Radius;
            double envelope = _radius * 1.1 + Math.Abs(_radialSpeed) * horizon + clearance;
        ...
        private bool ExcludesCurvedPath(FlightState state, double clearance, double horizon)
        {
            ...
            bool movingPrimary = TryCreate(state, _primaryIndex, out var primaryOrbit) && primaryOrbit._stableRadius;
            double centerError = movingPrimary ? primaryOrbit._accelerationError + primaryOrbit._centerAcceleration.Length
                : _centerAcceleration.Length * 1.1;
            double curveAcceleration = _angularSpeed * _angularSpeed * (_radius + Math.Abs(_radialSpeed) * horizon)
                + 2 * _angularSpeed * Math.Abs(_radialSpeed) + _centerAcceleration.Length * 1.1;
            int segments = Math.Max(1, (int)Math.Ceiling(horizon / 0.5));
            double step = horizon / segments;
            ...
            for (int sample = 1; sample <= segments; sample++)
            {
                ...
                double fraction = delta.LengthSquared > 1e-9 ? Math.Max(0, Math.Min(1, -Vector.Dot(previous, delta) / delta.LengthSquared)) : 0;
                double uncertainty = _radius * 0.1 + (_accelerationError + centerError) * (0.5 * time * time);
                double envelope = clearance + uncertainty + curveAcceleration * step * step / 8;
        ...
        private static bool TryCreateAround(FlightState state, int index, int primaryIndex, out OrbitalMotion motion)
        {
            ...
            double radialSpeed = Vector.Dot(velocity, offset.Unit);
            var tangent = velocity - offset.Unit * radialSpeed;
            double inwardAcceleration = -Vector.Dot(acceleration, offset.Unit);
            double expectedAcceleration = tangent.LengthSquared / Math.Max(1, offset.Length);

            bool orbital = primary.Radius                                      > 0                           &&
                           body.Radius                                         > 0                           &&
                           offset.Length                                       > 100                         &&
                           tangent.Length                                      > 10                          &&
