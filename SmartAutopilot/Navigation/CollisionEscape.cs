...
namespace SmartAutopilot.Navigation
{
    internal static class CollisionEscape
    {
        ...
        {
            selected = default;
            bool valid = requested.Phase == FlightPhase.Escape && dangerIndex >= 0 && dangerIndex < state.Obstacles.Count;
            if (!valid)
            {
                return false;
            }
        ...
        public static bool PathClear(FlightState state, Vector acceleration, Vector frameVelocity, Vector frameAcceleration,
            Func<Vector, Vector, double, bool> segmentClear)
        {
            ...
            for (int sample = 0; sample < 60; sample++)
            {
                ...
                double curvePadding = net.Length * step * step / 8;
                if (!segmentClear(position, next, curvePadding))
                {
                    return false;
                }

                double endTime = time + step;
                ...
                foreach (var body in state.Obstacles)
                {
                    ...
                    double fraction = delta.LengthSquared > 1e-9 ? Math.Max(0, Math.Min(1, -Vector.Dot(startOffset, delta) / delta.LengthSquared)) : 0;
                    double radius = body.PhysicalRadius + 15 + (net + frameAcceleration - body.Acceleration).Length * step * step / 8;
