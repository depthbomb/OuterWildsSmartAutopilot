...
namespace SmartAutopilot.Navigation
{
    ...
    internal sealed class RoutePlanner
    {
        ...
        public bool TryPlan(Vector start, Vector goal, IReadOnlyList<Obstacle> obstacles, Vector preferredDirection, out Waypoint next) => TryPlan(start, goal, obstacles, preferredDirection, out next, out _, out _);

        public bool TryPlan(Vector start, Vector goal, IReadOnlyList<Obstacle> obstacles, Vector preferredDirection, out Waypoint next, out Waypoint following, out double remainingDistance)
        {
            _goals.Clear();
        ...
        public bool TryPlanToTarget(Vector start, Vector target, double arrivalRadius, IReadOnlyList<Obstacle> obstacles, Vector preferredDirection, out Vector goal, out Waypoint next, out Waypoint following, out double remainingDistance)
        {
        ...
        private bool Search(Vector start, Vector target, double arrivalRadius, IReadOnlyList<Obstacle> obstacles, Vector preferredDirection, out Vector goal, out Waypoint next, out Waypoint following, out double remainingDistance)
        {
            ...
            double directDistance = double.PositiveInfinity;
            ...
            for (int i = 0; i < obstacles.Count; i++)
            {
                ...
                double shell = obstacle.Radius * 1.22 + 30;
                foreach (var direction in _directions)
                {
            ...
            for (int iteration = 0; iteration < count; iteration++)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    double estimate = _cost[i] + _heuristic[i];
                ...
                for (int i = 1; i < count; i++)
                {
                    ...
                    double bias = current == 0 ? edgeLength * 0.12 * (1 - Vector.Dot(edge / edgeLength, preferredDirection)) : 0;
                    double cost = _cost[current] + edgeLength + bias;
        ...
        private void AddGoal(Vector position, IReadOnlyList<Obstacle> obstacles)
        {
            ...
            {
                var obstacle = obstacles[i];
                bool tooClose = obstacle.Radius > 0 && (position - obstacle.Position).Length < obstacle.Radius + 50;
                if (tooClose)
                {
                    return;
                }
        ...
        public static bool SegmentClear(Vector start, Vector end, IReadOnlyList<Obstacle> obstacles)
        {
            ...
            for (int i = 0; i < obstacles.Count; i++)
            {
                ...
                double t      = lengthSquared > 1e-9 ? Math.Max(0, Math.Min(1, -Vector.Dot(offset, delta) / lengthSquared)) : 0;
