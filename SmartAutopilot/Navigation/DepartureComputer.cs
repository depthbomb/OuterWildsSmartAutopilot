...
namespace SmartAutopilot.Navigation
{
    ...
    internal sealed class DepartureComputer
    {
        ...
        public Vector Step(DepartureState state)
        {
            ...
            bool stalled = state.Time - _lastProgress > 12 || (state.Grounded && state.Time - _takeoffStart > 8);
            if (stalled)
            {
                Phase         = DeparturePhase.Failed;
            ...
            double speed = Phase == DeparturePhase.LiftOff ? 20 : Math.Min(80, Math.Sqrt(2 * reserve * Math.Max(30, state.ClearanceAltitude - state.Altitude)));
        ...
        {
            double speed = Math.Max(0, Vector.Dot(state.Velocity - state.SurfaceVelocity, state.Up));
            double reserve = Math.Max(1, state.ThrustReserve);
        ...
        public bool TryResume(DepartureState state, double suspendedAt)
        {
            ...
            bool nearby = CanSuspend && !state.Grounded && state.Time >= suspendedAt && state.Time - suspendedAt <= 15
        ...
        {
            double projection = Vector.Dot(surfaceOffset, up);
            double discriminant = projection * projection + safeRadius * safeRadius - surfaceOffset.LengthSquared;
