...
namespace SmartAutopilot.Navigation
{
    internal static class FlightValidation
    {
        ...
        {
            bool valid = state.Position.IsFinite          && state.Velocity.IsFinite       && state.ExternalAcceleration.IsFinite
            ...
            {
                bool usable = Finite(obstacle.Radius) && obstacle.Radius >= 0 && (obstacle.Radius == 0
                    || (obstacle.Position.IsFinite && obstacle.Velocity.IsFinite && obstacle.Acceleration.IsFinite
                    && Finite(obstacle.PhysicalRadius) && obstacle.PhysicalRadius >= 0));
                if (!usable)
                {
    ...
    internal sealed class FlightFaults
    {
        ...
        {
            bool allowed = !Locked && (!_faulted || time >= _retryAt);
            if (allowed)
            {
                _faulted = false;
            }
    ...
    internal static class CollisionBraking
    {
        ...
        {
            double speed   = (state.Velocity - frameVelocity).Length;
            double reserve = Math.Max(0.1, state.Thrust - (state.ExternalAcceleration - frameAcceleration).Length);
        ...
        {
            var acceleration = (frameVelocity - state.Velocity) / Math.Max(0.001, state.DeltaTime)
                + frameAcceleration - state.ExternalAcceleration;
