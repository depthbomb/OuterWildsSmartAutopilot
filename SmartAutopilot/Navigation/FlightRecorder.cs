...
namespace SmartAutopilot.Navigation
{
    internal readonly struct FlightSample
    {
        ...
        public override string ToString() => FormattableString.Invariant($"t={Time:F2}s; phase={Phase}; remaining={Remaining:F1}m; speed={Speed:F1}m/s; thrust={Thrust:F2}/{AvailableThrust:F2}; fuel={Fuel:P1}; targetRelativePosition=({Position.X:F1},{Position.Y:F1},{Position.Z:F1}); targetRelativeVelocity=({Velocity.X:F1},{Velocity.Y:F1},{Velocity.Z:F1}); relativeGravity=({Gravity.X:F2},{Gravity.Y:F2},{Gravity.Z:F2})");
    }
