...
namespace SmartAutopilot
{
    internal sealed class ShipCollisionGuard
    {
        ...
        public FlightCommand Step(FlightState state, FlightCommand command, IReadOnlyList<AstroObject> bodies, int dangerIndex)
        {
            ...
            Recovering = false;
            bool wasHolding = Active;
            Active = false;
            Scan(state, command, position);
            bool obstructed = Active;
            bool canNavigate = command.Phase != FlightPhase.Hold && command.Phase != FlightPhase.Arrived;
            bool distantHit = obstructed && _hitDistance > 500 && canNavigate && command.Phase != FlightPhase.Escape;
            if (distantHit)
            {
                _recoveryBounds = HullBounds(position);
                var frame = ReadFrame(state, _holdBody);
                bool clear = CollisionEscape.PathClear(state, command.Acceleration, frame.Velocity, frame.Acceleration, _segmentClear);
                obstructed = !clear;
            }
        ...
        private void Scan(FlightState state, FlightCommand command, Vector3 position)
        {
            ...
            {
                bool usable = _bodies[i] != null && state.Obstacles[i].Radius > 0;
                if (!usable)
                {
                    continue;
                }

                double gap = (state.Position - state.Obstacles[i].Position).Length - state.Obstacles[i].PhysicalRadius;
                if (gap < nearest)
                {
                    nearest  = gap;
            ...
            double distance = CollisionBraking.ProbeDistance(state, frame.Velocity, frame.Acceleration);
            ...
            int overlaps = Physics.OverlapBoxNonAlloc(bounds.center, bounds.extents, _overlaps, Quaternion.identity, OWLayerMask.physicalMask, QueryTriggerInteraction.Ignore);
            if (overlaps == _overlaps.Length)
            {
                Block(null, scanBody, "Collision scan crowded");
            }

            for (int i = 0; !Active && i < overlaps; i++)
            {
        ...
        private CollisionFrame ReadFrame(FlightState state, OWRigidbody body)
        {
            ...
            return CollisionFrame.Create(state.Position - FromUnity(body.GetWorldCenterOfMass()), FromUnity(body.GetVelocity()),
                FromUnity(body.GetAcceleration()), FromUnity(body.GetAngularVelocity()), FromUnity(body.GetAngularAcceleration()), radius);
        }
        ...
        private FlightCommand Recover(FlightState state, FlightCommand command, int dangerIndex, Vector3 position)
        {
            ...
            _recoveryBounds = HullBounds(position);
            var frame = ReadFrame(state, _holdBody);
            bool clear = CollisionEscape.TrySelect(state, command, dangerIndex, frame.Velocity, frame.Acceleration, _segmentClear, out var selected);
        ...
        private bool RecoverySegmentClear(Vector start, Vector end, double padding)
        {
            ...
            int overlaps = Physics.OverlapBoxNonAlloc(center, extents, _overlaps, Quaternion.identity, OWLayerMask.physicalMask, QueryTriggerInteraction.Ignore);
            if (overlaps == _overlaps.Length)
            {
                return false;
            }
            ...
            int count = Physics.BoxCastNonAlloc(center, extents, ToUnity(displacement.Unit), _hits, Quaternion.identity,
                (float)displacement.Length, OWLayerMask.physicalMask, QueryTriggerInteraction.Ignore);
        ...
        private Bounds HullBounds(Vector3 position)
        {
            ...
            {
                bool physical = collider != null && collider.enabled && !collider.isTrigger
                    && collider.attachedRigidbody == _ship.GetRigidbody();
                if (!physical)
                {
                    continue;
                }
        ...
        private void Sweep(Bounds bounds, Vector direction, double distance, OWRigidbody scanBody)
        {
            ...
            int count = Physics.BoxCastNonAlloc(bounds.center, bounds.extents, ToUnity(direction), _hits, Quaternion.identity,
                (float)Math.Min(distance, float.MaxValue), OWLayerMask.physicalMask, QueryTriggerInteraction.Ignore);
            if (count == _hits.Length)
            {
                Block(null, scanBody, "Collision scan crowded");
            ...
            for (int i = 0; i < count; i++)
            {
                bool relevant = !IsSelf(_hits[i].collider) && _hits[i].distance < nearest;
                if (relevant)
                {
        ...
        private void Block(Collider collider, OWRigidbody fallback, string reason)
        {
            ...
            {
                bool matches = body != null && body.GetOWRigidbody() == _holdBody;
                if (matches)
                {
                    var name = body.GetAstroObjectName();
                    string display = name == AstroObject.Name.CustomString ? body.GetCustomName() : AstroObject.AstroObjectNameToString(name);
        ...
        {
            string name = _scanBody != null ? _scanBody.name : "inertial space";
