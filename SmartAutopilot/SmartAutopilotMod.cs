...
namespace SmartAutopilot
{
    public sealed class SmartAutopilotMod : ModBehaviour
    {
        ...
        {
            float oldSpeed     = _speedLimit;
            float oldClearance = _clearance;
            ...
            bool navigationChanged = oldSpeed != _speedLimit || oldClearance != _clearance;
        ...
        {
            bool hadFlight = _computer != null && _flightContext != null && _logChanges;
            if (hadFlight)
            {
                string phase = _departure?.Active == true ? "Departure/" + _departure.Phase : _lastPhase?.ToString() ?? "Starting";
        ...
        internal bool TryReadInput(Autopilot pilot, out Vector3 input)
        {
            ...
            bool eligible = GameFields.IsShip.Get(pilot) && pilot.IsFlyingToDestination() && !pilot.IsMatchingVelocity();
            ...
            try
            {
                bool usable = !pilot.IsDamaged() && GameFields.Resources.Get(pilot).AreThrustersUsable();
                if (!usable)
                {
                ...
                bool banksUsable = thrusters.IsThrusterBankEnabled(ThrusterBank.Left) && thrusters.IsThrusterBankEnabled(ThrusterBank.Right);
                if (!banksUsable)
                {
                ...
                double fuel = GameFields.Resources.Get(pilot).GetFractionalFuel();
                ...
                bool phaseChanged = command.Phase != _lastPhase;
                int avoidanceIndex = command.Phase == FlightPhase.Detour ? _computer.DetourBodyIndex
                    : command.Phase == FlightPhase.Escape ? _computer.AvoidanceBodyIndex : -1;
                string collisionReference = _collision.Active ? "holding near " + _collision.ReferenceName
                    : _collision.Recovering ? "escaping near " + _collision.ReferenceName : null;
                bool statusChanged = phaseChanged || avoidanceIndex != _lastAvoidanceBody || collisionReference != _lastCollisionReference;
                if (statusChanged)
                {
                    ...
                    string displayName = avoiding == null ? "obstacle" : BodyDisplayName(avoiding);
                    _departure?.ShowNavigation(command.Phase, displayName);
                    if (_logChanges)
                    {
                        double distance = (_state.Position - _state.TargetPosition).Length;
                        double speed    = (_state.Velocity - _state.TargetVelocity).Length;
                        string avoidanceBody = avoidanceIndex >= 0 && _bodies[avoidanceIndex] != null
                            ? BodyDisplayName(_bodies[avoidanceIndex]) + " [" + avoidanceIndex + ":" + _bodies[avoidanceIndex].name + "]" : "none";
                        string routeStatus = command.Phase == FlightPhase.Hold ? " Reason: " + (_collision.Active ? _collision.Reason : _computer.RouteStatus) + "." : string.Empty;
                        int holdIndex = _computer.HoldBodyIndex;
                        string holdReference = command.Phase == FlightPhase.Hold && !_collision.Active
                            ? "holding near " + (holdIndex >= 0 && _bodies[holdIndex] != null ? BodyDisplayName(_bodies[holdIndex]) : "local space") : collisionReference;
                        string controlReference = holdReference != null ? " Control: " + holdReference + "." : string.Empty;
                ...
                bool waitingForRoute = command.Phase == FlightPhase.Hold || (_computer.RouteBlocked && command.Phase == FlightPhase.Escape);
                ...
                bool liningUp = command.Phase == FlightPhase.Detour || command.Phase == FlightPhase.Escape || command.Phase == FlightPhase.Hold;
                GameFields.LiningUp.Set(pilot, liningUp);
                GameFields.Approaching.Set(pilot, command.Phase == FlightPhase.Cruise);
                bool beganBraking = phaseChanged && command.Phase == FlightPhase.Braking;
                if (beganBraking)
                {
        ...
        internal void HandleFailure(Autopilot pilot, Exception exception)
        {
            ...
            try
            {
                bool first = _faults.Record(Time.time);
                StopFlight(pilot, _faults.Locked ? "Navigation fault; use manual controls\nAutopilot unavailable until the next loop"
                    : "Navigation fault; use manual controls\nWait 5 seconds before trying again");
                if (first)
                {
        ...
        private void Notify(string message)
        {
            ...
            try
            {
                ...
                notifications?.PostNotification(new NotificationData(NotificationTarget.Ship, message, 10, true), false);
            }
            catch (Exception exception)
            {
        ...
        private void UpdateHold(bool blocked, double fuel)
        {
            ...
            int holdBody = _computer.HoldBodyIndex;
            string holdName = holdBody >= 0 && _bodies[holdBody] != null ? BodyDisplayName(_bodies[holdBody]) : "local space";
            string reason = _collision.Active ? _collision.Reason + "; holding near " + _collision.ReferenceName : "No clear route; waiting near " + holdName;
            string action = _collision.Active ? "Rechecking path; cancel to fly manually" : "Select another target or cancel to fly manually";
        ...
        private void CaptureSample(Vector acceleration, double fuel, FlightPhase? phase)
        {
            ...
            string status = !phase.HasValue ? "Departure" : _collision.Active ? "Collision hold (" + _collision.ReferenceName + ")"
                : _collision.Recovering ? "Collision escape (" + _collision.ReferenceName + ")" : phase.Value.ToString();
        ...
        private void EnsureDeparture()
        {
            ...
            bool canCreate = !_departureUnavailable && _departure == null && ship != null
                && PlayerState.AtFlightConsole() && Locator.GetPromptManager() != null;
            if (canCreate)
            {
        ...
        private void BeginFlight(Autopilot pilot)
        {
            ...
            {
                bool supported = body.GetOWRigidbody() != null && body.GetAstroObjectType() != AstroObject.Type.None && body.GetAstroObjectName() != AstroObject.Name.DreamWorld;
                if (supported)
                {
                    _bodies.Add(body);
                    ...
                    {
                        double radius = hull.GetRadius(Time.time);
            ...
            string targetName = TargetDisplayName(_flightTarget);
            string primaryName = "none";
            for (int i = 0; i < _bodies.Count; i++)
            {
                var primary = _bodies[i].GetPrimaryBody();
                for (int depth = 0; primary != null && depth < 8; depth++)
                {
                    int index = _bodies.IndexOf(primary);
                    if (index >= 0 && index != i)
                    {
        ...
        private void LogBlockedRoute(ReferenceFrame frame)
        {
            ...
            for (int i = 0; i < _state.Obstacles.Count; i++)
            {
                var obstacle = _state.Obstacles[i];
                var offset   = obstacle.Position - _state.TargetPosition;
                double shipClearance = (obstacle.Position - _state.Position).Length - obstacle.Radius;
                bool relevant = obstacle.Radius > 0 && (offset.Length < obstacle.Radius + _state.ArrivalRadius + 500 || shipClearance < 500);
                if (relevant && _bodies[i] != null)
                {
                    var body = _bodies[i];
                    string name = body.GetAstroObjectName() == AstroObject.Name.CustomString ? body.GetCustomName() : body.GetAstroObjectName().ToString();
        ...
        private void LogAvoidance(int index)
        {
            ...
            double braking = Math.Max(_state.Thrust * 0.1, _state.Thrust - (_state.ExternalAcceleration - obstacle.Acceleration).Length) * 0.7;
            double stopping = closing * closing / (2 * braking);
            double closestTime = velocity.LengthSquared > 1e-9 ? Math.Max(0, Math.Min(velocity.Length / braking + 1, -Vector.Dot(offset, velocity) / velocity.LengthSquared)) : 0;
            double closestDistance = (offset + velocity * closestTime).Length;
            int primaryIndex = obstacle.PrimaryIndex;
            string primaryName = primaryIndex >= 0 && primaryIndex < _bodies.Count && _bodies[primaryIndex] != null
                ? _bodies[primaryIndex].GetOWRigidbody().name : "none";
            bool recognizedOrbit = OrbitalMotion.TryCreate(_state, index, out _);
            ...
            for (int depth = 0; depth < 2 && primaryIndex >= 0 && primaryIndex < _state.Obstacles.Count; depth++)
            {
        ...
        private void UpdateState(Autopilot pilot, ReferenceFrame frame)
        {
            ...
            for (int i = 0; i < _bodies.Count; i++)
            {
                ...
                double physicalRadius = GetPhysicalRadius(astro);
                ...
                double radius = physicalRadius + _clearance;
                ...
                bool isTarget = body == frame.GetOWRigidBody();
                if (isTarget)
                {
        ...
        {
            string display = frame.GetHUDDisplayName();
        ...
        private static string BodyDisplayName(AstroObject astro)
        {
            ...
            string display = name == AstroObject.Name.CustomString ? astro.GetCustomName() : AstroObject.AstroObjectNameToString(name);
        ...
        {
            var gravity = astro.GetGravityVolume();
            double radius = gravity != null ? GameFields.SurfaceRadius.Get(gravity) : 30;
            ...
            bool isSun = astro.GetAstroObjectName() == AstroObject.Name.Sun;
        ...
        {
            float value = config.GetSettingsValue<float>(key);
